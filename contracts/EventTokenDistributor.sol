...
contract EventTokenDistributor {
    ...
    function createEvent(uint256 _eventId, uint256 _maxPpl, uint256 _reward) public {
        ...
        require(eventExists[hEventId] == false, "Event already exist");
    ...
    //함수2. 토큰 지급(행사 참여)
    function distributeTokens(uint256 _eventId, uint256 _amount, string memory _groupId, string memory _userId) public {
        bytes32 hEventId = keccak256(abi.encodePacked(_eventId));
    ...
    function eventOver(uint256 _eventId, uint256 _remainToken) public returns (string memory) {
        ...
        if (_remainToken > 0) {
            tokenContract.burnFrom(address(this), _remainToken); // 남은 토큰 소각
        }
